...
def grade_laq(answer:str, max_marks:int, criteria:List[str]) -> Tuple[int,str]:
    ...
    def has_any(words): return any(w in answer.lower() for w in words)
    knowledge = simple_keyword_score(answer, kw_econ)
    analysis = 1.0 if has_any(["because","thus","therefore","hence"]) else 0.6
    evaln = 1.0 if has_any(["however","on the other hand","depends","limitations"]) else 0.5
    structure = 1.0 if has_any(["introduction","conclusion"]) else 0.7
    terms = 1.0 if has_any(["define","diagram","axes","curve","shifts","interpret"]) else 0.6
...
# macOS 'date' supports -v; if on Linux this may need adjustment
cat > data/slots.csv <<EOF
slot_id,date,time,subject,duration_min,price,status,booked_by_email
1,$(date -v+1d +"%Y-%m-%d" 2>/dev/null || date -d "+1 day" +"%Y-%m-%d"),17:00,Economics,${BOOKING_DURATION_MIN},${BOOKING_PRICE},available,
2,$(date -v+2d +"%Y-%m-%d" 2>/dev/null || date -d "+2 day" +"%Y-%m-%d"),18:00,Business Management,${BOOKING_DURATION_MIN},${BOOKING_PRICE},available,
EOF
...
# ====================
echo "🖼️ Logo Setup"
read -p "Enter full path to your logo (or press Enter to skip): " LOGO_PATH
if [ -n "$LOGO_PATH" ]; then
  if [ -f "$LOGO_PATH" ]; then
    cp "$LOGO_PATH" ./assets/skillnestlogo.png
    echo "✅ Logo copied to ./assets/skillnestlogo.png"
  else
    echo "⚠️ File not found at $LOGO_PATH. Skipping logo copy."
  fi
else
  echo "ℹ️ No logo provided. You can manually add it later to ./assets/skillnestlogo.png"
fi
